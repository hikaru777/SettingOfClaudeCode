---
name: produce-plan
description: 「どう宣伝すればいい」「売り方を考えたい」「どこに出す」と、宣伝の方針自体が決まっていない時に必ず使う。ユーザーが /produce-plan と打つのを待たず、この条件を満たしたら自分から起動すること。指定アプリ「固有」の宣伝戦略プランを立てる。アプリの型(macOSメニューバー/iOS/Web/CLI等)・武器・配布形態・ソース公開可否を判定し、効くチャネルだけを選定、コピー軸と動画コンセプトとローンチ順序まで設計してプランドキュメントをObsidian Valueに保存する。実行スキル /produce はこのプランを読んで動く。produce(汎用実行)の前段として固有戦略を固めるのが役割。
---
