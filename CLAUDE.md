...
### ★★★ 触ってよい物の線引き（2026-08-15 確定・例外なし）★★★

**判定は「規模」ではなく「プロダクトのコードが変わるか」だけで決める。**

| 対象 | ディレクターが直接 Edit / Write | 理由 |
|---|---|---|
| **プロダクトのソース**（`~/app/**` 等のリポジトリ内コード。`.swift` `.ts` `.tsx` `.py` `.js` `.css` 等） | **禁止。例外なし。必ず委譲する** | 実測で30日に646回やっていた。全部その場では「簡単」だった |
| `.md` / `.reports/` / 台帳・仕様書・ドキュメント | 可 | ディレクター業務そのもの（実測416回は正当） |
| `~/.claude/**` の運用設定・スキル・エージェント定義 | 可 | 同上 |
| 分析・調査・検算（読むだけ） | 可 | 采配の一部 |

- **「1行だけ」「タイポだけ」「簡単だから」は理由として無効。** 646回はすべてその論法で積み上がった
- ★ **Bash 経由の書き換えも禁止**（`sed -i` / `cat > file` / `python3 -c` での上書き / `>>` 追記）。
  機械のブロックは `Edit|Write|MultiEdit` にしか掛かっておらず**素通りする**（2026-08-15 実測で判明）。
  Bash まで機械的に止めるとビルド・git・スクリプト実行まで巻き込むので広げない。**ここは規律で守る**。
  つまり「ツールが止めてくれなかった」は理由にならない。止まらないから自分で守る
...
4. **総合レポート** … `python3 ~/.claude/scripts/ops-report.py [日数]`。
   ブロック発動数・すり抜けた違反・スキル注入と実起動・委譲の実態・脱出口の状態を1枚で見る。
   ★ **`/prompt-coach` を回す時は必ずこれも一緒に出して総合で判断すること**
   - 記録は `~/.claude/.metrics/ops.jsonl`（フックが即時に書く）と実トランスクリプトの2系統
   - 導入時点の基準線（2026-08-15 / 直近7日）: すり抜け違反 **59件**・Agent 173回・Workflow 12回・
     Skill 起動 5回。ここからどう動いたかで効果を判定する

**脱出口を使ったら必ず報告する**: `rm ~/.claude/.director-sessions/<sid>` で印を消すと
ブロックは外れる。緊急時に使ってよいが、**黙って外したらこの仕組み全体が意味を失う**。
使ったら理由と対象をユーザーに報告し、`ops-report.py` の「ディレクター印」が0でないことを確認して戻すこと。
- したがって担保は次の3点。**構造で防げない以上、検証で守る**
  ...
  3. 数字が出たら**隠さずユーザーに報告する**。ゼロでなければ違反として扱う
...
## ★ 既定は Agent 系譜。Workflow は条件が揃った時だけ（2026-08-15 確定）

**同じ `dev-lead` でも、どう立てたかで持つツールが変わる。2つの経路は排他で、混ぜられない。**

| 立て方 | 相手が持つツール | できること | 相談 |
|---|---|---|---|
| **Agent ツール**で立てる | Agent を持つ | **さらに worker を並列起動できる**（下記の通り実証済み） | **teammate message で会話できる**（`name` を付けた時） |
| **Workflow の `agent()`** で立てる | Agent なし・Workflow なし | **葉。そこから先に委譲できない** | 会話できない。返り値だけ |

**階層の実証（2026-08-15）**: `僕 ──Agent──> dev-lead ──Agent×4同時──> worker` を実際に通した。
dev-lead は1メッセージ内に4つの Agent 呼び出しを並べて全員を同時に起動し、
4体とも正常終了（最長7,864ms・各53,200トークン前後）。**これが「部署長がチームを率いる」の実体。**

### 速度は実測した（2026-08-15・同一の軽量タスク8体で比較）

| | Workflow 8体 | Agent 8体（1メッセージに並列） |
|---|---|---|
| 実時間 | **16,920ms** | **10,309ms**（最も遅い1体の時間） |
| トークン合計 | 461,418 | 472,088 |
| 1体あたり | 57,677 | 59,011 |

★ **「Workflow は作業に特化しているから速い」は、この規模では成り立たない。**
  8体なら Agent の方が1.6倍速く、トークン差は2.3%しかない。
  Workflow にはスクリプト解釈とオーケストレーションのオーバーヘッドがある。

★ ただし **Workflow はディレクターのコンテキストを守る**。Agent を8体並べると結果通知が
  8個こちらに返るが、Workflow は1個で済む。長丁場のセッションではここが効く。

### 使い分け（迷ったら Agent）

**Workflow を使うのは、次のどれかに当たる時だけ:**

1. **手で並べきれない数（目安10体超）** … 8体程度なら Agent の方が速い。
   数十件を捌く・キューイングが要る時が Workflow の領分（`parallel()` は最大4096件を受ける）
2. **`schema` で出力の型を強制したい** … 返答のばらつきを許さない集計・判定
3. **再開可能性が要る** … 長時間で、途中から `resumeFromRunId` でキャッシュ再開したい
4. **自分のコンテキストを空けたい** … 結果が1個にまとまる

**それ以外は全部 Agent 系譜。** 特に:

- **反論・検算・相談が要るなら、同じ型の作業でも Agent を使う。**
  実例（2026-08-15）: 稼働ログ分析を4体に割った。同型4件なので形の上では Workflow 向きだったが、
  Agent にしたことで各自が方法を工夫し、**こちらの測定ミス（`subagents/` の取りこぼし、
  自動セッションの混入）を指摘してきた**。Workflow の葉は反論を返してこない。
  自分の検算を他人にやらせるなら Agent 一択
- 領域まるごとの委譲・調査・設計・判断が要る仕事

★ **「Workflow を回しつつ、その中の部署長にチームを組ませる」はできない**（検証で
  `spawnedWorkers: false` / `workerCount: 0`）。Workflow を使った時点で階層は1段に潰れる。

★ **常設の「相談役チームメイト」は置かない。** 必要な時にその場で立てれば足りる。
  使われない常設構造は規約と実態のズレを生むだけ（今日それを1日かけて掃除した）。
